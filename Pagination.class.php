...
//	$url .= '&np='.$this->num_pages;
    ...
    if ($current_page != 1) {
      $html .= '<a class="first" title="First" href="' . $url.'&s=0">&laquo;</a>';
      $html .= '<a class="prev" title="Previous" href="' . $url.'&s='.($this->start - $this->display) . '">Предидущая</a>';
    } else {
      $html .= '<span class="disabled first" title="First">&laquo;</span>';
      $html .= '<span class="disabled prev" title="Previous">Предидущая</span>';
    }
    for ($i=$begin; $i<=$end; $i++) {
      if ($i != $current_page) {
        $html .= '<a title="' . $i . '" href="' . $url.'&s='.(($this->display * ($i - 1))) . '">' . $i . '</a>';
      } else {
        $html .= '<span class="current">' . $i . '</span>';
      }
    }
    if ($current_page != $this->num_pages) {
      $html .= '<a class="next" title="Next" href="' . $url.'&s='.($this->start + $this->display) . '">Следующая</a>';
      $last = ($this->num_pages * $this->display) - $this->display;
      $html .= '<a class="last" title="Last" href="' . $url.'&s='.($last) . '">&raquo;</a>';
    } else {
      $html .= '<span class="disabled next" title="Next">Следующая</span>';
